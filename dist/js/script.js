...
let dataForm = () => {
  const textHidden = document.getElementById("notifikasiSisaKarakter");
  const messageArea = document.getElementById("message");
  const maxLength = messageArea.maxLength;

  document.getElementById("message").addEventListener("input", () => {
    const typedText = document.getElementById("message").value.length;

    textHidden.removeAttribute("hidden");

    const sisaKarakter = maxLength - typedText;
  ...
  messageArea.addEventListener("blur", () => {
    textHidden.setAttribute("hidden", "true");
  });
};
